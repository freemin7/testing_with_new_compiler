...
void harmless() {
int old = (DEVICE_GPIO)->value;
(DEVICE_GPIO)->value = 1;
(DEVICE_GPIO)->value = 2;
(DEVICE_GPIO)->value = 4;
(DEVICE_GPIO)->value = 8;
(DEVICE_GPIO)->value = 16;
(DEVICE_GPIO)->value = old;
} 
...
(DEVICE_GPIO)->value = SR;
wait_sec(5);
SR = SR & (0xFFFFFFFF-(15<<4));
asm("LDC %0, SR;"
    :  
    : "r" (SR)
    :);
...
(DEVICE_AIC0)->brkadd = (uint32_t) &nothing ;
(DEVICE_AIC0)->ilevels = 15;
int i;
int j;
for(i=0;i<16;i++){
for(j=i+1;j<16;j++){
(DEVICE_AIC0)->ctrl0 = ((1<<i) + (1<<j));
nothing((1<<i) + (1<<j));
}
}

int255();
//int i = 1>>((DEVICE_AIC0)->rtc_sec_hi - (DEVICE_AIC0)->rtc_sec_hi); // This makes trouble, i later want to switch >> for / to see how division by 0 is handled
